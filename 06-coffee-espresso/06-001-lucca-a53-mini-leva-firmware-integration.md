...
| 8   | Pump-switch → `SNS` wiring                                         | Mains-rated hookup wire + faston/spade connectors + heat-shrink                                                                                                                                                                                                                                                                                                                                                  | **Not in kit.** Taps the pump switch's mains phase to ito `SNS`. Same teardown as the 06-011 PUMP-output fill tap — do both in one session.                                                                                                                                                                                                                                                                                                                                                                         |
...
3. **Wire the pump for phase-angle control.** Drive the vibratory pump from ito's on-board
   ...
   **cut the PUMP wire** (the unlabeled 3rd-from-top tab; confirm by meter — hot during _both_ a
   brew and a steam-boiler autofill) at the pump → route its **control-board side to `SNS`**, and
   drive the pump from **`Relay 1`**. ito `L`/`N` power comes off **`F`/PHASE + a Neutral tab** — ⚠️
   `F` is **always live** (120 V even in standby; the pad only toggles ON/standby, there is no
   switched rail). Optionally, a later step moves the **group solenoid (`1`/EV.GR) onto `Relay 2`**
   to unlock dosing / backflush / shot control — this hands the valve to ito (dosing stays off until
   you do it). → **Do steps 2–3 in the same machine-open session as
   [06-011](06-011-mini-v2-direct-plumb-in.md)'s fill-solenoid tap** — identical wiring area (both
   use the PUMP output), one teardown.
4. **Install the sensors.** Pressure sensor → **ADC** header — **tee a T-fitting into the brew
   ...
   **vibration-free** and cool-ish (125 °C-rated — heat is fine, vibration kills it; never at the
   ...
   (behind 3× 3 mm Allen screws) has three pins — **`+` = VCC (~14.3 V), `−` = GND, `o` = output**;
   the output is an **open-collector, 5 V-logic** signal (measured: +14.3 V VCC rail, but the signal
   swings 0↔5 V because the Vivaldi pulls it up to 5 V internally). Buffer that **output** into
   `IMPULSE` with **one CD4011B gate on ito's 5 V rail** (VDD pin 14 → ito 5 V, VSS pin 7 → GND; in
   A → 5 V, in B ← the GICAR `o` pin via a **Y-cable** shared with the Vivaldi; out → ito's flow
   input — reuse Dietmar's flow-meter cable, which already carries 5 V + GND). Keep the GICAR **`+`
   (~14.3 V) on the Vivaldi side ONLY — never to an ito input** (5 V + 0.5 V absolute max). ⚠️
   **blondica's _first_ build powered the logic on the 14.8 V rail and it interfered/failed** — the
   output is 5 V logic, so the interface runs at 5 V (t61709 #26). _(The NAND inverts, which is fine
   — transitions are what count.)_ **Even simpler (sandc, untested by him):** skip the IC —
   hard-wire the meter's `o` + `−` to both controllers, keep `+` on the Vivaldi, and desolder ito's
   **R6 (10 K pull-up) + C8 (100 N)** since the Vivaldi already provides them. Add an opto only for
   galvanic isolation. _(TSic → AUX and OLED → SPI/ENC are optional / for
