...
[Mini V2 modifications at a glance](_reference/mini-v2-modifications.md) — before/after of the
fluid + electrical paths across this project and [06-011](06-011-mini-v2-direct-plumb-in.md) (one
shared teardown).
...
| Flow display + flow-based control | ✅ in scope  | Reuses the machine's own stock GICAR meter (CD4011 NAND buffer, 5 V). |
...
AFTER  (ito interposed; leva! closes a feedback loop on the pump)
  ...
  The closed loop leva! runs continuously:
       ...
       ▼                                                                        │
  ★[PRESSURE SENSOR] ─► ito ADC ─► ★[ ito + leva! PID ] ─► phase angle ─► ★[ito Relay 1]     ─► [VIBE PUMP]
    measures brew P                 vs. a target profile    0°=full …            modulated (phase-angle;
                                    (pre-infuse→ramp→        ~115°=stop           solid-state relay, phase-fired)
                                     decline)
       also feeding leva!:
         stock GICAR meter ─►★[CD4011 NAND buffer @ 5 V]─► ito IMPULSE   (reuse machine's own meter; opto optional)
...
## The Progressive Preinfusion chamber — remove it

**Correction (2026-07): this machine shipped with a factory-fitted La Spaziale Progressive
Preinfusion chamber**, which the thread-derived notes above didn't account for (the community builds
either lacked it or didn't mention it). It matters for two reasons — one fatal to profiling, one
convenient.

**What it is.** A **spring-loaded piston in a cylinder** on a port on the _left_ of the group head.
Per La Spaziale's install manual: _"When the pump turns on … part of the water pressure is being
diverted to push the water down this 2nd path … this keeps the pressure low for approximately 6
seconds until the spring is fully compressed, then the pressure quickly ramps to its maximum."_ It
is a purely mechanical **hydraulic accumulator** in parallel with the puck.

**Why it must come out.** It is compliance inside leva!'s control loop, and worse, it **saturates**:
at a 1.5 bar electronic pre-infusion the piston barely moves, then when leva! ramps the pump the
diverted path vanishes and ~6 s of uncontrolled pre-infusion dumps into the middle of your profile.
La Spaziale say so directly — from their own FAQ, on running the chamber alongside programmable
(electronic) pre-infusion: _**"No. … it would cause a highly variable and unpredictable result."**_
leva! **is** programmable pre-infusion, so the two are manufacturer-documented incompatible. This is
almost certainly the source of the "pre-infusion swing" the Tuning section keeps warning about.

**The upside.** Removing it (5 mm hex on the brass side, or the chamber's own compression union)
frees a **factory pre-tapped, PTFE-sealed port straight into the brew chamber** — which becomes the
pressure-sensor mount (build step 4). So it deletes a profiling antagonist _and_ solves the sensor
plumbing in one reversible move. Fully reversible: refit the chamber (or a blanking plug) for stock
behaviour. Source: La Spaziale _Vivaldi Progressive Preinfusion Installation_ (Chris' Coffee) —
archived at `family/projects/electronics/espresso/_resources/`.
...
on-board Relays 1/2 this project uses to drive the pump/valve).
...
| 3   | Flow meter — **reuse stock GICAR** (kit Digmesa **not** installed) | Kit Digmesa (FHKSC / nano PP) stays in the box; the machine's own **GICAR** meter is tapped instead                                                                                                                                                                                                                                                                                                              | **Decision (was open, now closed):** do **not** plumb the kit meter into the tank→pump line — a second meter in series adds a restricting nozzle that cut free flow ~10 % (t56816 #31). Reuse the machine's stock GICAR meter via the CD4011 NAND buffer (row 7) → ito `IMPULSE`. Kit Digmesa kept as a spare.                                                                                                                                                                                                                                    |
| 4   | Sensor port adapter (maybe none)                                   | Thread the sensor into the **factory pre-infusion port** on the group head (build step 4)                                                                                                                                                                                                                                                                                                                        | **Correction (2026-07): this machine HAS a factory pre-tapped port** — the left-side group-head port the Progressive Preinfusion chamber occupies (PTFE-sealed, 5 mm-hex plug). Remove the chamber and thread the sensor in. **Confirmed (2026-07-16): the port is 1/8" BSP** (the seller supplied the replacement plug) — the **kit G1/8 fitting threads straight in, no adapter**. Seal with PTFE tape (a BSP "pipe plug" can be tapered). The tee-into-the-brew-line approach (t56816 #3) is the **fallback**, only if the thread ever fights. |
...
| 7   | **Stock-meter tap:** CD4011B NAND buffer (opto optional)           | One CD4011B gate as a buffer on **ito's 5 V rail**: VDD (pin 14) → ito 5 V, VSS (pin 7) → GND, in A (pin 1) → 5 V, in B (pin 2) ← the GICAR **output** pin via a **Y-cable** (shared with the Vivaldi board), out (pin 3) → ito's flow input. Reuse Dietmar's supplied flow-meter cable (it carries ito's 5 V + GND). **PC817 opto optional** — galvanic isolation only; blondica's bench-proven build omits it. | **Not in kit.** blondica's _first_ build powered the logic on the 14.8 V rail and it **interfered/failed** — the GICAR **output is a 5 V open-collector signal**, so run the logic at **5 V** (t61709 #26). GICAR pinout: **`+` = VCC ~14.3 V — Vivaldi-side ONLY, NEVER to an ito input (5 V + 0.5 V absolute max); `−` = GND (shared); `o` = output (shared).** Simpler alt (sandc, untested): skip the IC — share `o` + `−` to both controllers, keep `+` on the Vivaldi, desolder ito's R6 (10 K) + C8 (100 N).                               |
| 8   | Mains tap: pump → `SNS` + ito `L`/`N`                              | **6.3 × 0.8 mm fully-insulated faston** receptacles (F) + tabs (M) · mains-rated hookup wire · **bootlace ferrules** (for ito's screw clamps) · heat-shrink · a **ratcheting faston crimper** + **ferrule crimper**                                                                                                                                                                                              | **Not in kit. Non-destructive (2026-07): both ends unplug** — M5 is individual push-on plugs, the pump has 6.3 × 0.8 mm faston tabs, so this is pigtails, not cuts. Female fastons crimp to pigtails that push onto the board blades; a male tab feeds the pump's existing female plug from `SSR 1`. ito's `SNS`/`L`/`N` land in **screw clamps → ferrule the wire**. ⚠️ Mains crimps on a vibrating machine: use a **ratcheting** crimper, never pliers. Same teardown as the 06-011 fill tap — do both in one session.                          |
...
meter**, and (for now) the entire stock temperature-control board.

> **Measure / verify before ordering row 7.** Row 4 (sensor port) is now **confirmed 1/8" BSP** —
> the kit G1/8 fitting threads in, no adapter. Row 7 (stock-meter tap) depends on the **GICAR
> meter's pinout + signal levels** — verify by measuring the `#`/`T` (signal/ground) pins while the
> meter is connected to the Vivaldi (sandc). Rows 1–2 and 5 are in the kit; row 3's kit meter ships
> but isn't installed; rows 4 and 6–11 are the not-in-kit extras (6 and 11 are
...
2. **Inspect the grouphead.** Open it and confirm there's no spring-loaded valve / gicleur / S11/B
   ...
   yours. **⚠️ Check specifically for a factory-fitted Progressive Preinfusion chamber** (a small
   spring-loaded cylinder on a port on the _left_ of the group head — see "The Progressive
   Preinfusion chamber — remove it" section below). It is a **mechanical hydraulic accumulator** and
   La Spaziale's own install manual states it is **incompatible with programmable (electronic)
   pre-infusion** — which is exactly what leva! provides. **Remove it.** Its port then becomes the
   pressure-sensor mount (build step 4). On this machine (2026-07) the chamber was
   **factory-fitted**.
...
2. **Mount & power ito.** **Solder on the HLK-PM01 PSU** — it ships loose on purpose (mains-safety +
   ...
   orientation (L/N in, +5 V/GND out — check the manual footprint). **Done (2026-07-16): HLK-PM01
   soldered** — ito runs on its **own dedicated clean 5 V off mains** (chosen over tapping the
   machine's HDR-15-5). Fit the 4 adhesive feet + acrylic insulation shield; mount in a **non-metal
   or grounded enclosure — not metal screws** (insulation distance), away from boiler heat. Wire
   **N/L to the machine's mains N/L** (`F`/PHASE + Neutral — always live on this machine, which is
   fine: ito wants continuous power for zero-cross sensing); use a surge-protected outlet. Keep
   mains leads separated from low-voltage leads (EMC).
3. **Wire the pump for phase-angle control.** Drive the vibratory pump from ito's on-board
   **`Relay 1`** (clamp 1). leva!'s "Relays 1/2" are **solid-state** relays — they have to be: leva!
   ...
   [`_reference/mini-v2-control-board-wiring.md`](_reference/mini-v2-control-board-wiring.md)). **On
   this machine (verified 2026-07) M5 is a bank of individual push-on plugs** (each wire carries a
   little plastic ID marker: `A` / `2` / `1` / `F`; **PUMP is the unmarked one**), and the vibratory
   pump terminates in **6.3 × 0.8 mm faston tabs** — so **no cutting is required and the step is
   fully reversible.** Unplug the PUMP wire (confirm by meter — hot during _both_ a brew and a
   steam-boiler autofill); take its **control-board side to `SNS`** via a pigtail, and drive the
   pump from **`Relay 1` / `SSR 1`** via a second pigtail onto the pump's now-free faston. Reverse
   the whole thing by pulling the pigtails and re-seating the original plug. ito `L`/`N` power comes
   off **`F`/PHASE + a Neutral tab** — ⚠️ `F` is **always live** (120 V even in standby; the pad
   only toggles ON/standby, there is no switched rail). Optionally, a later step moves the **group
   solenoid (`1`/EV.GR) onto `Relay 2`** to unlock dosing / backflush / shot control — this hands
   the valve to ito (dosing stays off until you do it). → **Do steps 2–3 in the same machine-open
   session as [06-011](06-011-mini-v2-direct-plumb-in.md)'s fill-solenoid tap** — identical wiring
   area (both use the PUMP output), one teardown.
4. **Install the sensors.** Pressure sensor → **ADC** header. **On this machine, mount it in the
   factory pre-infusion port** — the S1 Mini Vivaldi II has a **plugged, PTFE-sealed port on the
   left of the group head** (factory 5 mm-hex brass plug; if a Progressive Preinfusion chamber is
   fitted it occupies this exact port — **remove the chamber**, see the note below). It reads
   **brew-chamber pressure directly**, needs **no tee and no cut plumbing**, and reverses by
   refitting the plug or chamber. The port is **1/8" BSP** (confirmed 2026-07-16 from the seller's
   replacement plug) — the **kit G1/8 fitting threads straight in, no adapter**; seal with PTFE
   tape. _Fallback (only if the thread ever fights):_ **tee a T-fitting into the brew line** and cap
   the spare branch for an optional gauge. Either way, mount **vibration-free** and cool-ish (125
   °C-rated — heat is fine, vibration kills it; never at the pump); the group head is the
   **least-vibrating** spot in the machine, so the port is also the best place mechanically. **Flow:
   reuse the machine's stock GICAR meter → `IMPULSE`** — the kit Digmesa is **not** plumbed in (a
   second in-series meter restricted free flow ~10 %, t56816 #31). The GICAR connector (behind 3× 3
   mm Allen screws) has three pins — **`+` = VCC (~14.3 V), `−` = GND, `o` = output**; the output is
   an **open-collector, 5 V-logic** signal (measured: +14.3 V VCC rail, but the signal swings 0↔5 V
   because the Vivaldi pulls it up to 5 V internally). Buffer that **output** into `IMPULSE` with
   **one CD4011B gate on ito's 5 V rail** (VDD pin 14 → ito 5 V, VSS pin 7 → GND; in A → 5 V, in B ←
   the GICAR `o` pin via a **Y-cable** shared with the Vivaldi; out → ito's flow input — reuse
   Dietmar's flow-meter cable, which already carries 5 V + GND). Keep the GICAR **`+` (~14.3 V) on
   the Vivaldi side ONLY — never to an ito input** (5 V + 0.5 V absolute max). ⚠️ **blondica's
   _first_ build powered the logic on the 14.8 V rail and it interfered/failed** — the output is 5 V
   logic, so the interface runs at 5 V (t61709 #26). _(The NAND inverts, which is fine — transitions
   are what count.)_ **Even simpler (sandc, untested by him):** skip the IC — hard-wire the meter's
   `o` + `−` to both controllers, keep `+` on the Vivaldi, and desolder ito's **R6 (10 K pull-up) +
   C8 (100 N)** since the Vivaldi already provides them. Add an opto only for galvanic isolation.
...
> **Tip — flash on the bench first.** De-risk the firmware step by doing **steps 5–6 (WLAN + leva!
> flash) on the bench** before the mains install. With the HLK-PM01 now soldered, feed a **5 V bench
> source into ito's 5 V rail** (leave the PSU's mains side dead on the bench), then do the mains
> install (steps 2–4) in the machine. Decouples the fiddly firmware work from the mains wiring.
...
### Non-destructive interim fit — order list

The teardown (2026-07) confirmed the whole interim build is **reversible**: M5 is push-on plugs, the
pump has faston tabs, and the pressure sensor threads into the factory pre-infusion port. So nothing
below cuts a wire or a pipe. Pressure profiling **does not need the flow meter at all** (leva!
profiles on pressure; flow only feeds the readout + `Flow Corr` bias), so the flow path is optional
and covered by the kit Digmesa straight into `IMPULSE` (it's a 5 V meter — no buffer, no
interposer).

**Already in the ito kit — do NOT order:** ito module, pressure sensor, Digmesa flow meter, OLED +
rotary encoder, FTDI, OKS grease, PTFE, 3× G1/8" fittings.

**To order (not in kit):**

| #   | Item                                   | Spec                                                                                         | Why / notes                                                                                                                                                                        |
| --- | -------------------------------------- | -------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A   | Faston connectors                      | **6.3 × 0.8 mm (¼") fully-insulated**, receptacles (F) **and** tabs (M), sized to wire gauge | Measured off the pump/board blades. F push onto the board's PUMP/`F`/`N` blades; one M feeds the pump's own female plug from `SSR 1`. An M+F insulated assortment kit covers both. |
| B   | Ratcheting faston crimper              | for insulated 6.3 mm terminals                                                               | **Mandatory** — a pliers-crimped mains joint on a vibrating machine becomes an intermittent that masquerades as a "leva! bug."                                                     |
| C   | Bootlace ferrules + ferrule crimper    | ferrules to suit the hookup wire                                                             | ito's `SNS` / `L` / `N` are **screw clamps** — ferrule the stranded wire before clamping.                                                                                          |
| D   | Mains-rated hookup wire + heat-shrink  | ~18–16 AWG, 105 °C/600 V, red + blue + green/yellow                                          | For the four pigtails (PUMP→SNS, F→L, N→N) + ground.                                                                                                                               |
| E   | ~~External 5 V supply~~ — **resolved** | **HLK-PM01 soldered (2026-07-16)** — ito runs on its own dedicated clean 5 V off mains       | **Nothing to order.** Chose the HLK-PM01 over tapping the machine's HDR-15-5. The one true solder/mains gate (step D2) is done.                                                    |
| F   | ~~Sensor port adapter~~ — **none**     | **Confirmed 1/8" BSP (2026-07-16)** — the kit G1/8 fitting threads straight in               | **Nothing to order.** Seal with PTFE tape (BSP "pipe plug" can be tapered). Closes BoM row 4.                                                                                      |

**Optional / conditional:**

- **Brass G1/8 blanking plug** — only if you want a stock, no-sensor state; not required, since
  refitting the pre-infusion chamber restores stock behaviour.
- **La Spaziale pump / 3-way seal-rebuild kit** — contingency, only if pre-flight §3 shows weeping
  seals.
- **Cheap tablet** for Status Monitor / the free ITO leva! companion app — if you don't already have
  one.

**Explicitly NOT needed for the interim build:** the `06-015` flow-tap interposer, the CD4011B
buffer, and any brew-line tee — all belong to the permanent GICAR-flow path, not to pressure
profiling.
...
- [x] Source ito kit from the EU (complete kit in hand: module + pressure sensor + flow meter +
      encoder)
- [x] Chose the flow approach: **reuse the stock GICAR meter** via a CD4011 NAND buffer on ito's 5 V
      rail → `IMPULSE` (kit Digmesa **not** installed — avoids the ~10 % in-series flow restriction)
- [ ] Source not-in-kit extras (CD4011B + cables [opto optional], SNS wiring, PETG, tablet); measure
      the brew line for the pressure T-fitting (no factory pre-tapped port to count on) + verify the
      GICAR pinout/levels
