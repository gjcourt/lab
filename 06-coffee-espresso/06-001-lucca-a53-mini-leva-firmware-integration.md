...
| 7   | **Stock-meter tap:** CD4011B NAND + PC817 opto                     | CD4011B CMOS NAND gate + PC817 optocoupler + a couple of custom cables; per blondica's build the interface ran off the meter's **~14.8 V supply** (t56816) — match the NAND's logic thresholds to however you pull up the GICAR open-collector output (an open design point — confirm against blondica's t56816 circuit before building) | **Not in kit.** Reads the stock GICAR meter's open-collector signal (0↔5 V) into ito `IMPULSE` — the NAND draws µA so it won't disturb the Vivaldi, and the opto gives galvanic isolation between the two controllers. **Never route the 14.3 V meter rail into an ito input** (5 V + 0.5 V absolute max). GICAR pinout: red = +VCC (14.3 V), white = signal, black = GND (t56816 #21/#26). (Replaces the old "flow-meter fittings" row — no in-line meter now.) |
