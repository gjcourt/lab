...
| 4   | Sensor port adapter (maybe none)                                   | Thread the sensor into the **factory pre-infusion port** on the group head (build step 4)                                                                                                                                                                                                                                                                                                                        | **Correction (2026-07): this machine HAS a factory pre-tapped port** — the left-side group-head port the Progressive Preinfusion chamber occupies (PTFE-sealed, 5 mm-hex plug). Remove the chamber and thread the sensor in. **Test-fit a kit G1/8 fitting first** — if it seats you need nothing; if the port is G1/4, buy **one G1/4→G1/8 adapter**. The tee-into-the-brew-line approach (t56816 #3) is the **fallback**, only if the port thread can't be adapted.                                                    |
...
4. **Install the sensors.** Pressure sensor → **ADC** header. **On this machine, mount it in the
   ...
   **brew-chamber pressure directly**, needs **no tee and no cut plumbing**, and reverses by
   refitting the plug or chamber. The port is a **parallel PTFE-sealed thread** — almost certainly
   **G1/8** (test-fit a kit G1/8 fitting first; if it's G1/4 like the hot-water valve, one G1/4→G1/8
   adapter closes it). _Fallback (only if the port thread can't be adapted):_ **tee a T-fitting into
   the brew line** and cap the spare branch for an optional gauge. Either way, mount
   **vibration-free** and cool-ish (125 °C-rated — heat is fine, vibration kills it; never at the
   pump); the group head is the **least-vibrating** spot in the machine, so the port is also the
   best place mechanically. **Flow: reuse the machine's stock GICAR meter → `IMPULSE`** — the kit
   Digmesa is **not** plumbed in (a second in-series meter restricted free flow ~10 %, t56816 #31).
   The GICAR connector (behind 3× 3 mm Allen screws) has three pins — **`+` = VCC (~14.3 V), `−` =
   GND, `o` = output**; the output is an **open-collector, 5 V-logic** signal (measured: +14.3 V VCC
   rail, but the signal swings 0↔5 V because the Vivaldi pulls it up to 5 V internally). Buffer
   that **output** into `IMPULSE` with **one CD4011B gate on ito's 5 V rail** (VDD pin 14 → ito 5 V,
   VSS pin 7 → GND; in A → 5 V, in B ← the GICAR `o` pin via a **Y-cable** shared with the Vivaldi;
   out → ito's flow input — reuse Dietmar's flow-meter cable, which already carries 5 V + GND). Keep
   the GICAR **`+` (~14.3 V) on the Vivaldi side ONLY — never to an ito input** (5 V + 0.5 V
   absolute max). ⚠️ **blondica's _first_ build powered the logic on the 14.8 V rail and it
   interfered/failed** — the output is 5 V logic, so the interface runs at 5 V (t61709 #26). _(The
   NAND inverts, which is fine — transitions are what count.)_ **Even simpler (sandc, untested by
   him):** skip the IC — hard-wire the meter's `o` + `−` to both controllers, keep `+` on the
   Vivaldi, and desolder ito's **R6 (10 K pull-up) + C8 (100 N)** since the Vivaldi already provides
   them. Add an opto only for galvanic isolation. _(TSic → AUX and OLED → SPI/ENC are optional / for
...
| E   | External **5 V ≥ 600 mA** supply      | USB charger + a USB pigtail, _or_ reuse the machine's on-board **Mean Well HDR-15-5** if it's spare                           | Lets you **skip soldering the HLK-PM01** onto ito (the one true solder/mains gate, step D2). **Confirm the HDR-15-5 isn't already committed** before relying on it.                |
| F   | Sensor port adapter                   | **likely none** — test-fit a kit G1/8 fitting into the pre-infusion port first; if the port is G1/4, **1× G1/4→G1/8 adapter** | Closes the last open unknown (BoM row 4). Confirm the port thread by test-fit before ordering.                                                                                     |
