...
2. **NC solenoid valve** — installed upstream of the regulator and the long run to the reservoir, so
   a downstream hose burst is closed off whenever the pump isn't drawing. Flow arrow on the valve
   body points downstream. Verify thread standard (NPT vs. BSPP) matches the adjacent fittings;
   transition with a brass adapter + PTFE tape (NPT) or fiber washer (BSPP) as needed.
...
- ⚠️ **The Mini V2 has no hard mains switch.** The soft-touch power pad is a logic input that only
  ...
  the wall. See the [control-board wiring reference](_reference/mini-v2-control-board-wiring.md) for
  the full terminal map and the always-live finding — this section defers to it for terminal detail.
...
> the pump runs, so refill happens only in those windows — there is **no catch-up between draws**.
> If the float valve's max flow (at your ~20–25 psi) is **below the pump's peak draw**, the tank
> drops a little each draw, the deficits **accumulate** (they never recover while the machine sits
> idle), and eventually the low-water switch trips — nuisance dry-run cutoffs under heavy use. The
> worst case is a **steam-boiler autofill** (the pump fills the boiler at ~free-flow, ≈600 mL/min);
> a brew is only ~2 mL/s of tank draw because the OPV bypass recirculates at the pump inlet.
> **Mitigate, cheapest first:** (a) size the fill ≥ pump draw — 25 psi, larger float-valve orifice,
> short/fat supply tube; **(b) add a ~30–60 s off-delay** on the solenoid (a time-delay relay, or an
> RC hold on the PSU) so the float catches up between draws — keeps the fail-safe, no smart plug; or
> (c) accept continuous-open via the smart-plug alternative above. **Bench test:** run several
> back-to-back shots then a steam session, watching the tank — holds/recovers = fine on flow; drifts
> down = add the off-delay.
...
little current for a solenoid). The coil PSU hangs off the **PUMP output**; ito hangs off
always-live mains:
...
— crack its case, wire its AC input (rated 100–240 V, so 120 V is fine) to the PUMP + NEUTRAL tabs
at the control board, and its 12 V DC output to the coil (**observe polarity**). ⚠️ A cracked-open
...
- **Regulator clamp** ([`.stl`](_reference/brackets/mini-v2-regulator-clamp.stl),
  [`.scad`](_reference/brackets/mini-v2-regulator-clamp.scad)) — split-ring C-clamp on the
  regulator's threadless barrel (Ø29.5 mm clamp area, 9.3 mm tall), single M3 pinch bolt, bolts to a
  **vertical wall** via two M5 slots. The ring's rear is flattened to the 3.2 mm the body allows
  behind the barrel; that thin back is the flex hinge.
- **Solenoid riser** ([`.stl`](_reference/brackets/mini-v2-solenoid-riser.stl),
  ...
  solenoid 26 mm off the **base plate**, coil-up, tucked next to the vibratory pump. Two M5 bolts
  pass up into the valve's tapped base (23 mm c/c, perpendicular to flow); two more bolt the riser
  straight down (drill new). All four are the same M5×12, via deep access bores so no long buried
  bolts.
...
and how to regenerate the meshes.
